AI-Powered Content Generation & CRM Automation System

1. Project Overview
This project is a no-code AI automation system built using:
n8n
Notion
Groq
Picsart
Objective
To automate the complete pipeline:
Lead / content brief input
AI content generation
Image generation
CRM-style tracking
Review & approval workflow

2. System Architecture
Workflow Flow
User fills Content Brief in Notion
n8n triggers automation
AI generates:
Blog
Social captions
Newsletter
Visual prompt
Picsart generates image
Data stored in Notion
Review workflow triggers automatically
Approved content moves to publishing stage

3. Database Structure (Notion)
Content Briefs
Name 
Brief ID
Campaign Name
Audience
Goal
Tone
Keywords
Blog Topic
Content Status
Generated Content
Name
Content ID
Source Brief ID
Blog Draft
Social Captions
Newsletter
Visual Prompt
Generated Image URL
Approval Status
Publish Status
Reviewer Notes
Visual Status

Workflow 1 — Content Generation Pipeline
Key Steps
Trigger from Notion
Extract brief data
Generate AI content
Parse structured output
Generate image via Picsart
Store output in Notion
Update content status

 AI Processing
The system uses AI to automatically generate:
Blog content
LinkedIn, Instagram, Twitter captions
Newsletter draft
Visual prompt for image generation
All outputs are structured in JSON format and parsed for automation.

Image Generation (Picsart)
Input: Visual Prompt
Process: AI image generation
Output: Image URL
Stored in Notion for review

Workflow 2 — Review & Approval Automation
Stages
Stage 1 — Content Generated
AI content + image created
Status = Draft
Stage 2 — Review
Content moves to review automatically
Reviewer checks quality
Stage 3 — Approval
Approved → Publish Status updated
Rejected → Sent for revision

Workflow Logic Summary
Stage
Action
New Brief
Trigger automation
AI Generation
Create content
Image Generation
Create visual
Review
Human validation
Approved
Move to publish
Rejected
Send for update

 Features Implemented
AI content generation
Automated image generation
CRM-style tracking system
Status-based workflow automation
End-to-end automation pipeline
No manual intervention required

Deliverables
Lead Generation Workflow JSON
https://drive.google.com/file/d/1479mv7wz_zE1xIz0cCFpyM6xFawPzygh/view?usp=sharing


Content Generation Workflow JSON
https://drive.google.com/file/d/1TAA9qHb04G6gBeeQXGbKVeCIcJao1ESh/view

Content Brief Notion Link 
https://www.notion.so/34b3b8197dee808abb35d85a81b2a435?v=34b3b8197dee809db1fa000c0902cabe


Generated Content Notion Link 
https://www.notion.so/34b3b8197dee806eb2e8d944a0a625f0?v=34b3b8197dee80808d5e000c727c01ce

Demo Video Link
https://drive.google.com/file/d/13lTh-bZyjm8GeVp4uO_CLwYAUx0K887c/view?usp=sharing\

Final Outcome
This system:
Automates content production
Reduces manual work
Maintains structured workflow
Improves scalability
Integrates AI with business automation



Conclusion
This project demonstrates how AI + No-Code Automation can be used to build a complete system that handles:
Input → Processing → Content Creation → Image Generation → Review → Publishing
in a fully automated and scalable manner.


